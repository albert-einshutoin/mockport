...
`-p 127.0.0.1:43101:43101` はホスト側で `127.0.0.1:43101` のみにポートを公開します。`--host 0.0.0.0` はコンテナ内のプロセスを全インターフェースで listen させ、Docker がそのトラフィックを転送できるようにするための指定です。どちらか一方ではなく、意図的に組み合わせた設定です。
