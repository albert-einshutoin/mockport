...
```bash
go run ./cmd/mockport run --config examples/zoho-oauth/mockport.yml
```
...
Optional deterministic user overrides (defaults: `mockport@example.test` / `Mockport User`) must be exported before starting Mockport:

```env
export ZOHO_USER_EMAIL=mockport@example.test
export ZOHO_USER_NAME="Mockport User"
```
