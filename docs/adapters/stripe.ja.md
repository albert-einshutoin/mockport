...
- `timeout` は即時の 504 レスポンス shape を返す。実レイテンシは server 全体の `X-Mockport-Delay`（`0`–`30000` ms、[Adapters](../site/adapters.ja.md) 参照）で注入する。
...
| --- | --- | --- |
| `payment_success` | `200` / Stripe 風 success object | scenario による sleep なし。実レイテンシは `X-Mockport-Delay` で制御（`0`–`30000` ms、[Adapters](../site/adapters.ja.md) 参照）。 |
| `payment_failed` | `402` / `card_declined` | scenario による sleep なし。実レイテンシは `X-Mockport-Delay` で制御（`0`–`30000` ms、[Adapters](../site/adapters.ja.md) 参照）。 |
| `auth_error` | `401` / `invalid_api_key` | scenario による sleep なし。実レイテンシは `X-Mockport-Delay` で制御（`0`–`30000` ms、[Adapters](../site/adapters.ja.md) 参照）。 |
| `rate_limited` | `429` / `rate_limited` | scenario による sleep なし。実レイテンシは `X-Mockport-Delay` で制御（`0`–`30000` ms、[Adapters](../site/adapters.ja.md) 参照）。 |
| `timeout` | `504` / `mockport_timeout` | 即時の `504` レスポンス shape のみ。scenario は **sleep や遅延処理を行わない**。実レイテンシは `X-Mockport-Delay` で制御（`0`–`30000` ms、[Adapters](../site/adapters.ja.md) 参照）。 |
