...
built-in scenario は `workflow-compatible` なローカルテスト向けの **安定したローカル契約** です。Mockport 内の決定的な挙動を定義しますが、provider の完全再現を意味しません。user YAML scenario はアプリ固有テスト向けにローカル挙動を積み増しできますが、provider compatibility を意味したり、文書化済みの gap を隠したりしてはいけません。
